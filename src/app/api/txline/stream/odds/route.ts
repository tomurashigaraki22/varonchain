...
export async function GET(req: NextRequest) {
  ...
  let upstream = await openTxlineStream("/odds/stream", jwt, apiToken);

  if (upstream.status === 401 || upstream.status === 403) {
    const renewedJwt = await fetchGuestJwt();
    upstream = await openTxlineStream("/odds/stream", renewedJwt, apiToken);
  }

  if (!upstream.ok || !upstream.body) {
    return new Response(
      JSON.stringify({ error: `Upstream odds stream failed: ${upstream.status}` }),
